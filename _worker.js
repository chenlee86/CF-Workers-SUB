...
let subConfig = "https://raw.githubusercontent.com/chenlee86/CF-Workers-SUB/main/sub-config.ini"; //订阅配置文件
...
export default {
	async fetch(request, env) {
		...
		if (!(token == mytoken || token == fakeToken || url.pathname == ("/" + mytoken) || url.pathname.includes("/" + mytoken + "?"))) {
			...
			if (订阅格式 == 'base64' || 订阅格式 == 'quanx' || token == fakeToken) {
...
<head>
    ...
    <style>
        ...
        .link-label { font-weight: bold; width: 100px; flex-shrink: 0; }
        .link-url { 
            color: blue; 
        ...
        .editor {
            width: 100%;
            height: 60vh; /* 默认占据屏幕60%高度 */
            min-height: 400px;
            margin: 10px 0;
            padding: 12px;
            border: 1px solid var(--border-color);
            border-radius: 4px;
            font-family: inherit; /* 使用继承的系统默认字体 */
            font-size: 13px;
            line-height: 1.5;
            resize: vertical;
            box-sizing: border-box;
        ...
        .btn-copy { background: #666; font-size: 12px; padding: 4px 10px; }
        ...
        .footer-info { font-size: 12px; color: #999; text-align: center; margin-top: 20px; }
    </style>
...
<body>
    ...
    <div id="qrcode-modal">
        <div id="qrcode"></div>
        <p style="margin-top:10px; font-size:13px;">已自动复制链接</p>
    </div>
